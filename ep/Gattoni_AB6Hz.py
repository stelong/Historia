...
def initStates():
    sizeStates = 18
    states = zeros(shape=(sizeStates,), dtype=float)
...
def initParams():
    sizeParameters = 11
    parameters = zeros(shape=(sizeParameters,), dtype=float)
...
def initConsts(parameters):
    sizeConstants = 71
    constants = zeros(shape=(sizeConstants,), dtype=float)
...
def computeAlgebraic(constants, states, voi):
    sizeAlgebraic = 85
    algebraic = zeros(shape=(sizeAlgebraic,), dtype=float)
...
def computeRates(constants, algebraic, states, voi):
    sizeRates = 18
    rates = zeros(shape=(sizeRates,), dtype=float)
...
def plot_calcium(sol):
    plt.plot(sol.t, 1e3*sol.y[12,:], c='r')
...
def main():
    ...
    y0 = initStates()
    tspan = [0, 170]
    t = linspace(0, 170, 1701)

    start_time = time.time()

    sol = solve_ivp(fun=lambda t, y: model(t, y, c, p), t_span=tspan, y0=y0, t_eval=t)
