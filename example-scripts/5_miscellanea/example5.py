...
def main():
	...
	f_emul = lambda x: np.array([emul.predict(x.reshape(1, -1))[0] for emul in emulator]).ravel() # this function returns a vector of predictions at a specific parameter set
	...
	x = np.ones((len(xlabels),), dtype=float) # a vector of ones gives back the reference, unperturbed PV-loop
	y = f_emul(x) # the provided emulators in this example have fractions of the unit as input parameter values
	t, lvv, lvp = upvl(edp, y) # this function returns an uncertain PV-loop given a set of predicted LV features
	ax1, ax2, ax3 = pvloop((ax1, ax2, ax3), t, lvv, lvp, 'k', '--', 1.0, 'control') # plotting
	...
	for i, p in enumerate(perturbation_list):
		...
		ax1, ax2, ax3 = pvloop((ax1, ax2, ax3), t, lvv, lvp, lsc[i], '-', 1.5, r'{:g}$\times${}'.format(p, param))
