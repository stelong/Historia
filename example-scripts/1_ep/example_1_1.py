...
def fun(axes, rat, color):
	...
	ep_params_path = 'data/parameters.json' # path to parameter file

	E = solep.EPSolution(rat, hz, ep_params_path) # instantiate the class of EP model solution
	...
	contr_params_path = '../2_cellcontr/data/parameters.json' # path to parameter file

	C = solcontr.CONTRSolution(rat, calcium, contr_params_path) # instantiate the class of CONTR model solution
	...
	C_ss = solcontr.CONTRSolution(rat, Cai, contr_params_path) # instantiate the class of CONTR model steady-state solution
	C_ss.steadystate_sol() # calculate the steady-state solution using the given equally spaced log calcium values

	Tref = C_ss.constant['Tref'] # used constants can be retrieved this way by knowing the constant name (see "example_3.py" for more details)
