...
def main():
	...
	available_parameters = list(E.constant.keys()) # list of available constants to be treated as parameters
	...
	param_ref_val = E.constant[param] # store the reference value for the selected parameter
	...
	for i, (V, Ca) in enumerate(zip(V_list, Ca_list)):		
		axes[0].plot(t, V, c=lsc[i], lw=2, label=f'{param} = {param_vals[i]:.6f}')
		axes[1].plot(t, Ca, c=lsc[i], lw=2, label=f'{param} = {param_vals[i]:.6f}')
