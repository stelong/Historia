...
def main():
	...
	available_parameters = list(C.constant.keys()) # list of available constants to be treated as parameters (same as C_ss.constant)
	...
	param_ref_val = C.constant[param] # store the reference value for the selected parameter
	...
	param_vals = np.linspace(p_inf*param_ref_val, p_sup*param_ref_val, N) # vector of perturbed parameter values
	...
	for i in range(N):
		...
		new_params_dict = {param: param_vals[i]} # here you can add more altered parameters if you want
	...
	for i, (FpCa, T) in enumerate(zip(FpCa_list, T_list)):		
		axes[0].plot(pCai, FpCa, c=lsc[i], lw=2, label=f'{param} = {param_vals[i]:.6f}')
		axes[1].plot(t, T, c=lsc[i], lw=2, label=f'{param} = {param_vals[i]:.6f}')
