...
def save_Ca_Ta_lambda_dlambdadt(path_to_sim, path_to_out):
	...
	for elem in range(108):
		...
		desu.write_txt(Ca_v[:, 1:], '%f', path_to_out + 'Ca_elem_{}'.format(elem+1))
		desu.write_txt(Ta_v[:, 1:], '%f', path_to_out + 'Ta_elem_{}'.format(elem+1))
		desu.write_txt(lambd_v[:, 1:], '%f', path_to_out + 'lambda_elem_{}'.format(elem+1))
		desu.write_txt(dlambd_v[:, 1:], '%f', path_to_out + 'dlambda_dt_elem_{}'.format(elem+1))
...
def overlap(l, c):
	beta0 = c[6]
	return np.max([0, 1.0 + beta0*(np.min([l, 1.2]) + np.min([np.min([l, 1.2]), 0.87]) - 1.87)])
...
def rhs(t, y, calcium, lambd, dlambd, c):
	...
	ca50 = ca50ref*(1.0 + beta1*(np.min([1.2, np.max([0.8, lambdi])]) - 1.0))
...
def cell_contraction(path_to_in, elem, node):
	...
	Ca = desu.read_txt(path_to_in + 'Ca_elem_{}'.format(elem+1), 'float64')[node]
	Ta = desu.read_txt(path_to_in + 'Ta_elem_{}'.format(elem+1), 'float64')[node]
	lambd = desu.read_txt(path_to_in + 'lambda_elem_{}'.format(elem+1), 'float64')[node]
	dlambd = desu.read_txt(path_to_in + 'dlambda_dt_elem_{}'.format(elem+1), 'float64')[node]

	Y = solve_ivp(fun=lambda t, y: rhs(t, y, Ca, lambd, dlambd, c), t_span=tspan, y0=Y0, method='BDF', t_eval=t, max_step=1.0)
...
def compute_ATP(path_to_in):
	...
	XB = []
	for elem in range(108):
		XB.append( desu.read_txt(path_to_in + 'XB_elem_{}'.format(elem+1), 'float64') )
