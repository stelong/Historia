...
class CONTRSolution:
	...
	*https://physoc.onlinelibrary.wiley.com/doi/full/10.1113/jphysiol.2012.231928
	...
	def solver_sol(self, p_dict=None):
		...
		Y = solve_ivp(lambda t, y: Land2012.computeRates(t, y, self.Cai, self.constant), t_span=tspan, y0=Y0, method='BDF', t_eval=self.t, max_step=1.0)
		self.Y = Y.y
	...
	def steadystate_sol(self, p_dict=None):
		...
		TRPN_ss, trpn_EC50, XB_ss, xb_EC50, F_ss, pCa50, nH, hl = Land2012.computeSteadystate(self.Cai, self.constant)
